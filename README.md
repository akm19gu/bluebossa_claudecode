...
視線が左上から右下へ流れるように組んである。
- 左上：見出し（入口）
- 右：ロゴと同じ正方形の「ステージ」。見出しより少し下げてあり、小節ごとに中身が替わる
- 左下：ステートメント本文。小節ごとに一文ずつ現れ、最後のフレームで全文が読める
- 右下：最後に出る M3 の情報と URL（出口）

画面の端に置くのは、左上の名前、右上の曲名、ステージの下の拍のルーラー（拍ごとに目盛りが光り、再生位置の線が進む）だけにしている。
...
| 0.72 | 1 | Cm7 | Experimental digital jazz | 中央の線が開いて塗りの鍵盤が現れる（曲の最初の和音は 2 拍目の 1.29 秒）。ドラムが入る 2.64 秒で鍵盤がすばやく横に流れる |
| 3.33 | 2 | Fm7 | Rhythm and harmony | 鍵盤のグリッサンドでワイプ。バンドが目指すものの図解（この録音のデータではない）。上段はストレートの列と、そこから前後にずれるドランク（dilla）の列。下段は四分音つきの 12 鍵で、拍ごとに C → E♭−50¢ → F → A♭+50¢ と、鍵盤のあいだにも止まる |
| 5.94 | 3 | Dm7♭5 | Beyond convention | **ii** |
| 8.54 | 4 | G7 | ii – V – I | **V**。続いて、I が来るはずの空き枠 |
| 11.15 | 5 | Cm7 | Acoustic and digital | **I** が 70ms 遅れて傾いて枠に着地（drunk）→ 打ち消し線 → 枠が広がってロゴの絵になり、1・5・2・POST の順に文字が灯る |
| 12.46 | 5（3 拍目） | — | Grooves | アルバム名。ステージの下（右下）に M3-2026秋 の情報と URL |
...
フォントは Inter / Cormorant Garamond / Noto Sans JP（いずれも SIL OFL 1.1、`assets/fonts/licenses/`）。
